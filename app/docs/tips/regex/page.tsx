        ...
        <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
            ...
            <header className="space-y-4">
                ...
                <h1 className="text-4xl font-black text-slate-800 uppercase tracking-tighter">
                    Регулярные <span className="text-primary">выражения</span>
                </h1>
                <p className="text-lg text-slate-500 leading-relaxed">
                    ...
                    позволяет сказать программе: <i>«Найди мне слово "Артикул", после которого идет пробел и ровно 3 любые цифры»</i>.
                </p>
            </header>
            ...
            <section className="space-y-6">
                ...
                <p className="text-slate-600 text-sm leading-relaxed">
                    Regex состоит из обычных символов (которые ищутся как есть) и <b>Метасимволов</b> (специальных команд). 
                    Например, буква <code>A</code> ищет букву «А», а команда <code>\d</code> ищет <b>любую цифру</b>. 
                    ...
                    <CardBody className="p-0">
                        <div className="p-6 border-b border-slate-800 space-y-2">
                            ...
                            <div className="font-mono text-lg text-slate-300">
                                Товар <span className="bg-primary/30 text-white px-1 rounded">ART-90210</span> (на складе)
                            </div>
                        </div>
                            ...
                            <div className="space-y-2 pt-4">
                                <div className="flex gap-3 items-start">
                                    ...
                                    <div className="text-xs text-slate-400 leading-relaxed">
                                        <b>Точное совпадение:</b> Программа ищет ровно эти 4 символа подряд (буквы A, R, T и дефис).
                                    </div>
                                </div>
                                <div className="flex gap-3 items-start">
                                    <div className="w-12 shrink-0 font-mono text-green-400 text-right mt-0.5">\d</div>
                                    <div className="text-xs text-slate-400 leading-relaxed">
                                        <b>Токен (Сущность):</b> Означает «любая цифра от 0 до 9».
                                    </div>
                                </div>
                                <div className="flex gap-3 items-start">
                                    <div className="w-12 shrink-0 font-mono text-purple-400 text-right mt-0.5">{`{5}`}</div>
                                    <div className="text-xs text-slate-400 leading-relaxed">
                                        <b>Квантификатор (Количество):</b> Говорит программе, что предыдущий символ (цифра) должен повториться ровно 5 раз.
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <section className="space-y-6">
                ...
                <p className="text-slate-600 text-sm leading-relaxed">
                    Квантификатор всегда ставится <b>после</b> символа или токена и указывает, сколько раз этот символ может встречаться.
                </p>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <QuantifierCard symbol="*" name="Ноль или больше" desc="Символа может не быть вообще, а может быть бесконечно много." />
                    <QuantifierCard symbol="+" name="Один или больше" desc="Символ обязательно должен быть хотя бы один раз." />
                    <QuantifierCard symbol="?" name="Ноль или один" desc="Делает предыдущий символ необязательным (или есть, или нет)." />
                    <QuantifierCard symbol="{n}" name="Ровно N раз" desc="Например, \d{3} найдет ровно 3 цифры (123)." />
                    <QuantifierCard symbol="{n,}" name="От N и больше" desc="Например, \w{2,} найдет слова длиной от 2 букв и больше." />
                    <QuantifierCard symbol="{n,m}" name="От N до M раз" desc="Например, \d{2,4} найдет от 2 до 4 цифр подряд." />
                </div>
                    ...
                    <CardBody className="p-5 flex gap-4 items-start">
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-xs text-amber-700 leading-relaxed">
                                По умолчанию <code>*</code> и <code>+</code> очень жадные — они захватывают максимально длинный кусок текста. <br/>
                                Если у вас текст <code>"А", "Б", "В"</code> и вы напишете <code>".*"</code> (кавычка, любой текст, кавычка), он захватит всё от первой до последней кавычки: <b>"А", "Б", "В"</b>. <br/>
                                Чтобы он остановился на первой же закрывающей кавычке (стал ленивым), добавьте знак вопроса: <code>".*?"</code>. Результат: <b>"А"</b>.
                            </p>
                        </div>
                    </CardBody>
                </Card>
            ...
            <section className="space-y-6">
                ...
                <p className="text-slate-600 text-sm leading-relaxed">
                    Это специальные коды, которые обозначают группы символов. Заглавная буква всегда означает <b>противоположность</b> строчной.
                </p>
                ...
                <div className="border border-slate-200 rounded-xl overflow-hidden bg-white">
                    ...
                    {/* Базовые */}
                    <TokenRow token="." desc="Любой символ вообще (кроме переноса строки)." />
                    <TokenRow token="\d" desc="Любая цифра (от 0 до 9)." />
                    <TokenRow token="\D" desc="Всё, что УГОДНО, только НЕ цифра (буквы, пробелы, знаки)." />
                    <TokenRow token="\w" desc="Словесный символ (буквы a-z, А-Я, цифры 0-9 и знак подчеркивания _)." />
                    <TokenRow token="\W" desc="НЕ словесный символ (пробелы, точки, запятые, дефисы)." />
                    <TokenRow token="\s" desc="Пробельный символ (пробел, табуляция, невидимые отступы)." />
                    <TokenRow token="\S" desc="НЕ пробел (любой видимый символ)." />
                    
                    {/* Наборы */}
                    <TokenRow token="[abc]" desc="Набор: найдет либо 'a', либо 'b', либо 'c'. Любой ОДИН символ из скобок." />
                    <TokenRow token="[^abc]" desc="Отрицание: найдет любой символ, КРОМЕ 'a', 'b' и 'c'." />
                    <TokenRow token="[a-z]" desc="Диапазон: любая маленькая английская буква." />
                    <TokenRow token="[А-Яа-я]" desc="Диапазон: любая русская буква (большая или маленькая)." />
                    <TokenRow token="[0-9]" desc="Аналог \d (любая цифра)." />
                    
                    {/* Позиции */}
                    <TokenRow token="^" desc="Начало строки. ^Привет найдет слово 'Привет' только если оно стоит в самом начале." />
                    <TokenRow token="$" desc="Конец строки. Пока$ найдет слово 'Пока' только в самом конце текста." />
                    <TokenRow token="\b" desc="Граница слова. \bКот\b найдет слово 'Кот', но проигнорирует его внутри слова 'Который'." />
                    
                    {/* Группировка и логика */}
                    <TokenRow token="(abc)" desc="Группа. Объединяет символы. (abc)+ найдет 'abcabcabc'." />
                    <TokenRow token="(a|b)" desc="ИЛИ. Найдет либо 'a', либо 'b'. Например (cat|dog) найдет кота или собаку." />
                    <TokenRow token="(?:abc)" desc="Группа без захвата. Объединяет, но не сохраняет в память (экономит ресурсы)." />
                    
                    {/* Спецсимволы */}
                    <TokenRow token="\" desc="Экранирование. Если вам нужно найти саму точку (а не любой символ), напишите \." />
                    <TokenRow token="\n" desc="Перенос на новую строку." />
                    <TokenRow token="\t" desc="Символ табуляции (Tab)." />
                    
                    {/* Продвинутые */}
                    <TokenRow token="(?=abc)" desc="Позитивный просмотр вперед. Найдет позицию ПЕРЕД 'abc', не захватывая само 'abc'." />
                    <TokenRow token="(?!abc)" desc="Негативный просмотр вперед. Убеждается, что впереди НЕТ 'abc'." />
                    <TokenRow token="(?<=abc)" desc="Позитивный просмотр назад. Найдет позицию ПОСЛЕ 'abc'." />
                    <TokenRow token="(?<!abc)" desc="Негативный просмотр назад. Убеждается, что сзади НЕТ 'abc'." />
                </div>
            </section>
                ...
                <CardBody className="p-8 flex flex-col md:flex-row items-center gap-6 text-center md:text-left">
                    ...
                    <div className="flex-1 space-y-2">
                        ...
                        <p className="text-sm text-slate-600 leading-relaxed max-w-xl">
                            ...
                            Мы настоятельно рекомендуем сайт <b>regex101.com</b> — это лучший в мире бесплатный тренажер. 
